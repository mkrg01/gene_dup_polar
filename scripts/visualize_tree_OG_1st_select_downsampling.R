...
sp_OG <- sp_OG %>%
	...
	pivot_longer(!sci_name, names_to = 'OG', values_to = 'n_gene') %>%
	mutate(OG = factor(OG, levels=selected_OG_df$OG_id))
...
p <- ggtree(tree_trait) +
	geom_tiplab(aes(label=paste0('italic(', label, ')~italic()')), parse=TRUE, size=1.0) +
	geom_fruit(geom=geom_bar, mapping=aes(y=label, x=lat_abs_max), stat='identity', axis.params=c(axis='x', text.size=1.5, vjust=1), grid.params=list(), offset=0.75, pwidth=0.4)

p <- p +
	new_scale_fill() +
	geom_fruit(data=gene_category_df, geom=geom_tile, mapping=aes(y=sci_name, x=OG, fill=n_gene), offset=0.01, pwidth=2, width=9, axis.params=c(axis='x', text.angle=90, text.size=1.5, hjust=1, line.color='white')) +
...
ggsave(args$output, plot=p, width=6, height=4)