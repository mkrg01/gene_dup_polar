...
trait_og_df <- sp_OG %>%
    ...
    dplyr::select(organism_id, OG_name, num, lat_abs_max, Polar_fish)

trait_og_df$OG_name <- factor(trait_og_df$OG_name, levels=selected_OG_df$OG_name)
...
ggsave(args$output, plot=p, width=9, height=11)