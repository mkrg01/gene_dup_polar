...
p <- p +
	new_scale_fill() +
	geom_fruit(data=gene_category_df, geom=geom_tile, mapping=aes(y=sci_name, x=OG_name, fill=n_gene), offset=0.01, pwidth=1.1, width=10, axis.params=c(axis='x', text.angle=90, text.size=1, hjust=1, line.color='white')) +
...
ggsave(args$output, plot=p, width=5, height=6.5)